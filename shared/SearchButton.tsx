  ...
  return (
    <>
      <motion.button
        ...
        aria-label={t({ ru: "Поиск", kk: "Іздеу" })}
            ...
            >
              ...
              <div className="flex items-center gap-3 rounded-2xl2 border border-line bg-card px-4 py-3.5 shadow-card">
                ...
                <input
                  ...
                  placeholder={t({
                    ru: "Поиск по разделам, проектам и новостям…",
                    kk: "Бөлімдер, жобалар мен жаңалықтар бойынша іздеу…",
                  })}
                  className="flex-1 bg-transparent text-[15px] text-ink outline-none placeholder:text-ink-faint"
                  aria-label={t({ ru: "Поисковый запрос", kk: "Іздеу сұранысы" })}
                />
                <button
                  type="button"
              ...
              {query.trim() && (
                ...
                >
                  {results.length === 0 ? (
                    ...
                    >
                      {t({ ru: "Ничего не найдено по", kk: "Ештеңе табылмады:" })} «{query.trim()}»
                    </motion.div>
                  ) : (
                    <motion.ul
                    ...
                    >
                      {results.map((r, i) => (
                        <motion.li
                          key={`${r.category.ru}-${r.href}-${r.title}`}
              ...
              {!query.trim() && (
                <div className="mt-3 px-1 text-center text-[12px] text-ink-faint">
                  {t({
                    ru: "Начните вводить запрос — например, «карта», «ИИ» или «ЖКХ»",
                    kk: "Сұранысты енгізіңіз — мысалы, «карта», «ЖИ» немесе «ТКШ»",
                  })}
                </div>
              )}
            </motion.div>
